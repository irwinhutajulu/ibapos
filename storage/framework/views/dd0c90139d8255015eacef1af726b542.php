...
<div class="bg-white border rounded-md">
  <div class="p-3 border-b flex items-center justify-between">
    <div class="font-semibold">New Stock Adjustment</div>
    <a href="<?php echo e(route('stock-adjustments.index')); ?>" class="text-sm underline">Back</a>
  </div>
  <div class="p-3">
    <?php if($errors->any()): ?>
    ...
    <?php endif; ?>
  <form method="POST" action="<?php echo e(route('stock-adjustments.store')); ?>" x-data="adjForm()">
      <?php echo csrf_field(); ?>
      <div class="grid md:grid-cols-2 gap-3">
        <div>
          <label class="block text-sm">Reason</label>
          <input type="text" name="reason" value="<?php echo e(old('reason')); ?>" class="w-full border rounded p-2" />
        </div>
        <div>
          <label class="block text-sm">Note</label>
          <input type="text" name="note" value="<?php echo e(old('note')); ?>" class="w-full border rounded p-2" />
        </div>
      </div>
            ...
            <tbody>
              <template x-for="(row, idx) in rows" :key="idx">
                <tr class="border-b" :class="rowError(idx) ? 'bg-red-50' : ''">
                  ...
                  <td class="align-top">
                    <input type="number" step="0.001" :name="`items[${idx}][qty_change]`" x-model="row.qty_change" class="w-full border rounded p-1" placeholder="e.g. -2 or 5" :class="!row.qty_change || row.qty_change == 0 ? 'border-red-400' : ''" />
                    <div class="text-xs text-red-600" x-show="!row.qty_change || row.qty_change == 0">Non-zero</div>
                  </td>
                  <td>
                    <input type="number" step="0.0001" :name="`items[${idx}][unit_cost]`" x-model="row.unit_cost" class="w-full border rounded p-1" placeholder="optional for positive adj" />
                  </td>
                  <td>
...
function adjForm() {
  return {
    rows: [ { product_id: '', product_name: '', qty_change: '', unit_cost: '', note: '' } ],
    suggest: {},
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Data IBA POS\IBAPOS\resources\views/adjustments/create.blade.php ENDPATH**/ ?>