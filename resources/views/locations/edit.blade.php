...
<div class="p-6">
    <div class="mb-6">
        <div class="flex items-center gap-2 mb-4">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Location</h1>
        </div>
        <p class="text-gray-600 dark:text-gray-400">Update location information and user assignments</p>
    </div>
        ...
        <form action="{{ route('locations.update', $location) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="address" 
                          ...
                          rows="3"
                          class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                          placeholder="Enter the complete address of this location">{{ old('address', $location->address) }}</textarea>
                @error('address')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="border border-gray-300 dark:border-gray-600 rounded-md p-4 max-h-64 overflow-y-auto">
                    @php
                        $selectedUserIds = old('user_ids', $location->users->pluck('id')->toArray());
                    @endphp
    ...
    <div class="mt-6 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
        <h3 class="text-sm font-medium text-blue-800 dark:text-blue-200 mb-2">Location Information</h3>
        <div class="text-sm text-blue-700 dark:text-blue-300 space-y-1">
            <p><span class="font-medium">Created:</span> {{ $location->created_at->format('d M Y, H:i') }}</p>
            <p><span class="font-medium">Last Updated:</span> {{ $location->updated_at->format('d M Y, H:i') }}</p>
            <p><span class="font-medium">Currently Assigned Users:</span> {{ $location->users->count() }} user(s)</p>
        </div>
    </div>
</div>
