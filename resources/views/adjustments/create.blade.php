...
<div class="bg-white border rounded-md">
  <div class="p-3 border-b flex items-center justify-between">
    ...
    <a href="{{ route('stock-adjustments.index') }}" class="text-sm underline">Back</a>
  </div>
  <div class="p-3">
    @if($errors->any())
    ...
    @endif
  <form method="POST" action="{{ route('stock-adjustments.store') }}" x-data="adjForm()">
      @csrf
      <div class="grid md:grid-cols-2 gap-3">
        <div>
          <label class="block text-sm">Reason</label>
          <input type="text" name="reason" value="{{ old('reason') }}" class="w-full border rounded p-2" />
        </div>
        <div>
